...
impl Imp<Enum> for ViewTrait {
    fn strategy(node: &Enum) -> Option<TraitStrategy> {
        ...
        let from_view_arms = node.variants.iter().map(|variant| {
            ...
            if variant.value.is_some() {
                quote! {
                    Self::ViewType::#variant_ident(v) => Self::#variant_ident(::mimic::core::traits::View::from_view(v))
                }
            } else {
                quote! {
...
impl Imp<Tuple> for ViewTrait {
    fn strategy(node: &Tuple) -> Option<TraitStrategy> {
        ...
        let to_view_fields = indices.iter().map(|i| {
            ...
            quote! {
                ::mimic::core::traits::View::to_view(&self.#index)
            }
        });
        ...
        let from_view_fields = indices.iter().map(|i| {
            let index = syn::Index::from(*i);
            quote! {
                ::mimic::core::traits::View::from_view(view.#index)
            }
        });
...
fn field_list(view_ident: &Ident, fields: &FieldList) -> TokenStream {
    let to_pairs: Vec<_> = fields
        ...
        .map(|field| {
            ...
            quote! {
                #ident: ::mimic::core::traits::View::to_view(&self.#ident)
            }
        })
        .collect();
    ...
    let from_pairs: Vec<_> = fields
        ...
        .map(|field| {
            let ident = &field.ident;
            quote! {
                #ident: ::mimic::core::traits::View::from_view(view.#ident)
            }
        })
        .collect();
