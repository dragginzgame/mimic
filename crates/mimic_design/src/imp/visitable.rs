    ...
    fn tokens(node: &Map, t: Trait) -> Option<TokenStream> {
        let inner = quote! {
            for (k, v) in self.0.iter() {
                ...
                ::mimic::def::visit::perform_visit(visitor, k, &visitor_key);
                ::mimic::def::visit::perform_visit(visitor, v, &visitor_key);
            }
        };
        let q = drive_inner(&inner);
    ...
    fn tokens(node: &Set, t: Trait) -> Option<TokenStream> {
        let inner = quote! {
            for (i, item) in self.0.iter().enumerate() {
                ...
                ::mimic::def::visit::perform_visit(visitor, item, &visitor_key);
            }
        };
        let q = drive_inner(&inner);
...
fn quote_variant(value: &Value, ident: &Ident) -> TokenStream {
    ...
    match value.cardinality() {
        Cardinality::One => quote! {
            Self::#ident(value) => ::mimic::def::visit::perform_visit(visitor, value, #name),
        },
        Cardinality::Opt => quote! {
            Self::#ident(option_value) => if let Some(value) = option_value {
                ::mimic::def::visit::perform_visit(visitor, value, #name);
            },
        },
...
fn drive_inner(inner: &TokenStream) -> TokenStream {
    ...
    quote! {
        fn drive(&self, #visitor: &mut dyn ::mimic::def::visit::Visitor) {
            #inner
        }
    }
}
...
fn quote_value(var: &syn::Expr, cardinality: Cardinality, name: &str) -> TokenStream {
    match cardinality {
        Cardinality::One => quote! {
            ::mimic::def::visit::perform_visit(visitor, &#var, #name);
        },
        Cardinality::Opt => quote! {
