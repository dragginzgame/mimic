...
impl Imp<EntityId> for IntoTrait {
    fn tokens(node: &EntityId, t: Trait) -> Option<TokenStream> {
        ...
        let q = quote! {
            fn into(self) -> mimic::def::types::Ulid {
                self.ulid()
            }
        };
        ...
        tokens.extend(
            Implementor::new(&node.def, t)
                .set_tokens(q)
                .add_trait_generic(quote!(mimic::def::types::Ulid))
                .to_token_stream(),
        );
        ...
        let q = quote! {
            fn into(self) -> mimic::def::types::Key {
                self.key()
            }
        };
        ...
        tokens.extend(
            Implementor::new(&node.def, t)
                .set_tokens(q)
                .add_trait_generic(quote!(mimic::def::types::Key))
                .to_token_stream(),
        );
