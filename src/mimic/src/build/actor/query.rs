...
fn query_load(builder: &mut ActorBuilder) {
    let q = quote! {
        ...
        ) -> Result<Vec<::mimic::db::types::DataRow>, ::mimic::Error> {
            // look up canister_id
...
fn query_load_response(builder: &mut ActorBuilder) {
    let q = quote! {
        ...
        pub fn query_load_response(
            ...
            let res: Result<Vec<::mimic::db::types::DataRow>, ::mimic::Error> = DB.with(|db| {
...
fn query_delete_response(builder: &mut ActorBuilder) {
    let q = quote! {
        ...
        pub fn query_delete_response(
            ...
            let res: Result<::mimic::query::DeleteResponse, ::mimic::Error> = DB.with(|db| {
                let res = executor.execute(db)?;
